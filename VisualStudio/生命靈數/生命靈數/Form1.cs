...
namespace 生命靈數
{
    public partial class Form1 : Form
    {
        ...
        {
            Getstart(dateTimePicker1.Value.ToString());
        }

        private string Getstart(string s)
        {
            DateTime dtDate = DateTime.Parse(s);
            string countalldate = dateTimePicker1.Value.Year.ToString() + dateTimePicker1.Value.Month.ToString() + dateTimePicker1.Value.Day.ToString();
            string res = "";
            string Ans = "";
            Dictionary<int, string> result = new Dictionary<int, string>();
            int Num = 0;
            foreach (var V in countalldate)
            {
                int all;
                string b = V.ToString();
                int.TryParse(b, out all);
                Num += all;
            }
           while (Num > 9)
            {
            ...
            switch (dtDate.Month)
            {
                case 1:
                    ...
                    {
                        res = "摩羯座";
                        label3.Text = "你的星座是" + res;
                        result = CreateCapricorn();
